...
export function ProductCarouselClient({
  ...
  return (
    <motion.section
      ...
      animate={isInView ? "show" : "hidden"}
      variants={sectionVariants}
      // ✅ key fix: never allow horizontal bleed
    ...
    >
      ...
      {/* ✅ key fix: clip carousel internals */}
      <div className="relative overflow-hidden">
        <Carousel opts={{ align: "start" }} className="w-full">
          <motion.div variants={listVariants} initial="hidden" animate={isInView ? "show" : "hidden"}>
            {/* ✅ key fix: remove negative margins and use gap instead */}
            <CarouselContent className="ml-0 gap-2 sm:gap-4">
              {products.map((product) => (
                <CarouselItem
                  key={product.id}
                  // ✅ no pl-* needed now; gap handles spacing
                ...
                >
                  <motion.div
                    ...
                    className="h-full"
                  >
                    <ProductCard className="py-0" product={product} onQuickView={() => handleQuickView(product)} />
                  </motion.div>
                </CarouselItem>
              ))}
            </CarouselContent>
