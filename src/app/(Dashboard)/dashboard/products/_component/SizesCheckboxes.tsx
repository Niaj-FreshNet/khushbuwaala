            ...
            {selectedSizes.map((size) => (
              <label key={size} className="flex items-center gap-1 text-sm text-gray-900 font-bold">
                    <input
                        ...
                        checked={form.getValues('variants').some((v) => v.size === Number(size))}
                        onChange={(e) => {
                            const variants = form.getValues('variants');
                            if (e.target.checked) {
                                form.setValue('variants', [
                                    ...variants,
                                    { id: uuidv4(), size: Number(size), price: 0, stock: 0, sku: '', unit: 'ml' },
                                ]);
                            } else {
                                form.setValue('variants', variants.filter((v) => v.size !== Number(size)));
                            }
                        }}
                    />
